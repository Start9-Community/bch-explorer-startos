...
export const current = VersionInfo.of({
  ...
  releaseNotes: {
    ...
    es_ES:
      'Añade la acción Reparar MariaDB. Tras un apagado sucio o un disco lleno, MariaDB puede entrar en bucle por un tc.log corrupto (Bad magic header). Un Rebuild de StartOS deja ese archivo en el volumen; esta acción lo elimina y reinicia, conservando los datos indexados. También sirve los logos de los pools desde la imagen del frontend en lugar de proxificar a bchexplorer.cash (ese proxy ahora devuelve 403). Las páginas de direcciones sobre Flowee funcionan con cashaddr (incluido el formato con token bchtest:z…). Los prevouts que faltan en Flowee ya no hacen fallar con un 500 toda la lista de transacciones de una dirección.',
    de_DE:
      'Fügt die Aktion „MariaDB reparieren“ hinzu. Nach einem unsauberen Shutdown oder vollem Datenträger kann MariaDB wegen einer beschädigten tc.log in einer Absturzschleife hängen (Bad magic header). Ein StartOS-Rebuild lässt die Datei auf dem Volume; diese Aktion löscht sie und startet neu, die indexierten Daten bleiben. Liefert Mining-Pool-Logos außerdem aus dem Frontend-Image statt über den Proxy zu bchexplorer.cash (der jetzt 403 liefert). Adressseiten über Flowee funktionieren mit cashaddr (auch token-fähig, bchtest:z…). Fehlende Flowee-Prevouts lassen die Transaktionsliste einer Adresse nicht mehr komplett mit 500 scheitern.',
    pl_PL:
      'Dodaje akcję Napraw MariaDB. Po nieczystym wyłączeniu lub zapełnieniu dysku MariaDB może zapętlać się na uszkodzonym tc.log (Bad magic header). Rebuild w StartOS zostawia ten plik na wolumenie; ta akcja go usuwa i restartuje, zachowując zindeksowane dane. Serwuje też loga puli z obrazu frontendu zamiast proxy do bchexplorer.cash (to proxy zwraca teraz 403). Strony adresów przy Flowee działają z cashaddr (w tym z tokenowym bchtest:z…). Brakujące prevouty z Flowee nie kończą już całej listy transakcji adresu błędem 500.',
    fr_FR:
      "Ajoute l'action Réparer MariaDB. Après un arrêt brutal ou un disque plein, MariaDB peut boucler sur un tc.log corrompu (Bad magic header). Un Rebuild StartOS laisse ce fichier sur le volume ; cette action le supprime et redémarre, en conservant les données indexées. Sert aussi les logos de pools depuis l'image frontend au lieu de les proxifier vers bchexplorer.cash (ce proxy renvoie désormais 403). Les pages d'adresse via Flowee fonctionnent avec cashaddr (y compris le format token bchtest:z…). Les prevouts manquants de Flowee ne font plus échouer toute la liste des transactions d'une adresse avec une erreur 500.",
  },
  migrations: {
    up: async ({ effects }) => {},
    down: IMPOSSIBLE,
